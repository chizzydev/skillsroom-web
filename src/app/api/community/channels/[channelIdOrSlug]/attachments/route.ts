...
import { deleteChatMedia, storeChatMedia } from "@/lib/chat-media-storage";
...
export async function POST(request: Request, context: Context) {
  ...
  try {
    ...
    if (!reserve.ok || !attachmentId) throw new Error(reserved.error?.message ?? "Attachment upload could not start.");
    const stored = await storeChatMedia({ file, attachmentId });
    ...
    if (storageKey) await deleteChatMedia(storageKey);
    ...
    return Response.json({ ok: false, error: { code: "CHAT_ATTACHMENT_UPLOAD_FAILED", message: error instanceof Error ? error.message : "Attachment upload failed." } }, { status: 400 });
  }
}
